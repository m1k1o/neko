...
M1 按“媒体性能与网络基础设施优先、UI 重构后置”的顺序推进。前 1–6 项完成并通过发布基线后，才开始第 7 项 UI 重构；这样 UI 变更不会与编码器、端口映射或自适应策略的快速迭代相互干扰。第 7 项采用可回滚的渐进式迁移，不改变信令、媒体协议和服务端端口契约。
...
| 3 | 自适应质量策略 | 显式 profile 生成质量梯度，复用带宽估计器；后续加入队列压力、RTT/jitter/丢包输入 | 压力下降档、恢复升档，切换原因可观测 |
...
| 7 | UI 渐进重构（M1 后段） | 在媒体、网络和发布基线稳定后，先提取 TypeScript WebRTC/连接 SDK，再逐页拆分房间、连接、媒体和 UI 状态；保留旧页面作为回退路径 | 不改变信令/媒体协议；新旧 UI 可独立切换和回滚；Chromium、认证代理、FRP/TURN、端口和性能回归通过 |

## 9. 里程碑与成功标准

1. **M1：Chromium 性能、认证代理与单端口连通性**：仅支持 Chromium；支持 Linux x86_64 和 Windows x86_64 Docker Desktop/WSL2；默认 UDP MUX、TCP/TURN/FRP 回退、启动预检、带认证的 HTTP CONNECT/SOCKS5 出站代理、媒体背压和质量策略完成，并通过性能门槛；在这些基础能力收敛后，于 M1 后段执行 UI SDK/状态拆分，不改媒体协议。
