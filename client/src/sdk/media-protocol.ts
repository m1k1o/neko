...
export type MediaInput =
  | { event: 'mousemove'; x: number; y: number; epoch?: number }
  | { event: 'wheel'; x: number; y: number; controlKey?: boolean; epoch?: number }
  | { event: 'mousedown' | 'mouseup' | 'keydown' | 'keyup'; key: number; epoch?: number }
...
function createPacket(opcode: number, length: number, epoch = 0) {
  assertEpoch(epoch)
  const buffer = new ArrayBuffer(HEADER_BYTES + EPOCH_BYTES + length)
  const view = new DataView(buffer)
  view.setUint8(0, opcode)
  // The v3 server protocol uses network byte order for the header and body.
  view.setUint16(1, EPOCH_BYTES + length, false)
  view.setUint32(3, Math.floor(epoch / 0x100000000), false)
  view.setUint32(7, epoch >>> 0, false)
  return { buffer, view }
}
...
export function encodeMediaInput(input: MediaInput): ArrayBuffer {
  switch (input.event) {
    case 'mousemove': {
      ...
      const { buffer, view } = createPacket(MEDIA_OPCODE.MOVE, 4, input.epoch)
      view.setUint16(11, input.x, false)
      view.setUint16(13, input.y, false)
      return buffer
    }
    case 'wheel': {
      assertInteger('x', input.x, -0x8000, 0x7fff)
      assertInteger('y', input.y, -0x8000, 0x7fff)
      const { buffer, view } = createPacket(MEDIA_OPCODE.SCROLL, 5, input.epoch)
      view.setInt16(11, input.x, false)
      view.setInt16(13, input.y, false)
      view.setUint8(15, input.controlKey ? 1 : 0)
      return buffer
    }
    case 'keydown':
    case 'keyup': {
      ...
      const { buffer, view } = createPacket(opcode, 4, input.epoch)
      view.setUint32(11, input.key, false)
      return buffer
    }
    case 'mousedown':
    case 'mouseup': {
